...
- **Monitor** in the mixer's master bar plays the mix to connected headphones, during soundcheck and while recording, so a take can be heard as it is captured. It stays silent unless a wired, USB or Bluetooth headset is connected: on the loudspeaker the phone microphone would feed straight back, and monitoring closes by itself within half a second of headphones being unplugged. Bluetooth output runs roughly 0.2 s behind, which suits checking a take and not playing along; a wired or USB headset is much closer. The monitor never delays the mix thread, so a sink that cannot keep up drops audio rather than disturbing the recording.
...
- Recordings contain MP4 video (H.264 or HEVC) and a 48 kHz stereo AAC mix at the encoder’s advertised maximum bitrate, capped at 512 kb/s. A lossless 48 kHz / 24-bit stereo WAV master is enabled by default and saved to the chosen folder, or `Music/GearCam` by default (about 1.04 GB/hour). Disable it in the mixer if space is limited. Separate input stems are not implemented.
...
- Inputs other than the phone microphone have a **USB whine filter** where the phone mic has its gate: it cancels the comb of tones at multiples of 1 kHz, the USB frame rate, that an adapter's own electronics add to its signal. One sine per harmonic is adapted to the tone actually present and subtracted, so it follows the device's clock drift and adds no delay. Measured on a BOYA BY-K4 recording with no input connected: 22 dB or more off every harmonic within 0.12 s, taking the channel from −82 to −88 dBFS, its broadband floor. The notches are a few Hz wide at 1 kHz and about ±15 Hz at the top harmonics. On a commercial track the mix level changes by 0.01 dB and the median critical band by 0.01 dB; the cost falls only on a steady tone lying within a few Hz of an exact 1 kHz multiple, which is cancelled like the whine (worst measured case: one band 1.2 dB down). Leave it off on inputs that do not whine.
...
- **Start recording on MIDI play** (Settings, or the audio recorder's Config) starts a take when any USB MIDI device sends Start or Continue, while GearCam is on screen. It only starts takes; Stop is ignored. Android carries a USB MIDI 1.0 port on the same kernel driver as that device's audio, and direct capture detaches that driver, so MIDI from the interface being captured is unavailable during a take and is reopened about a second after it. For the same reason the audio recorder does not run its live waveform between takes while this option is on; it starts with the take.
...
The overlapping-circles filter icon in the camera’s top bar opens **Original · no filter**, **Black & white**, **B&W Noir**, **Warm Film**, **Cool Chrome**, **Sepia**, **Fade**, and **Color Accent**. **Original is selected by default**; no creative filter is applied until you choose one. Color Accent keeps one hue and turns everything else monochrome, with more contrast than Black & white and none of Noir's crush or vignette. Choosing it opens a rainbow slider for the kept hue, which updates the preview as you drag; pick it again to reopen the slider. Hues within about 18° of the choice are kept in full and fade out by 40°; unsaturated and very dark pixels stay mono, so grey and black areas are unaffected. Select Original again to turn filtering off. The active choice is remembered, and the icon is accented while a filter is enabled. UI color themes do not change the recorded image.
...
- USB Audio Class 1 and 2 input, uncompressed PCM or 32-bit float, at exactly 48 kHz are supported. UAC2 devices get their clock set to 48 kHz and GearCam checks it; a device locked to another rate reports an error. Compressed formats, sample-rate conversion at USB ingress and USB output are not implemented. MIDI is read through Android, not this driver.
