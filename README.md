...
- Choose one of seven color themes in **Settings → Color theme** or the audio recorder’s **Config → Color theme**: Mint (default), Ocean, Violet, Rose, Amber, Graphite or Paper. The choice persists across launches. Camera controls use a compact, centred top bar with camera switch, orientation lock, exposure, an overlay button (grid → zebras 95% → RGB histogram → off), filters, popup and settings; face detection is disabled and its settings are removed.
