...
namespace uac {
    ...
    struct uac_output_terminal {
        ...
        uint8_t  bAssocTerminal;
        uint8_t  bSourceID;
        uint8_t  bCSourceID; // UAC2 clock entity; 0 for UAC1
        uint8_t  iTerminal;
    };
     ...
     */
    /**
     * UAC2 Tables 4-6 and 4-7: Clock Source and Clock Selector
     */
    struct uac2_clock_entity {
        uint8_t subtype;
        uint8_t bClockID;
        std::vector<uint8_t> sources; // selector inputs
    };
    ...
    struct uac_as_general {
        ...
        uint8_t  bDelay;
        uint16_t wFormatTag;
        uint8_t  bNrChannels; // UAC2 only; UAC1 keeps it in the format descriptor
    };
