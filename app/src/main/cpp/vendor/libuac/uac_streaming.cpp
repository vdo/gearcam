...
namespace uac {
            ...
            case LIBUSB_TRANSFER_COMPLETED:
                for (int packet_id = 0; packet_id < transfer->num_iso_packets; ++packet_id) {
                    ...
                    if (packet->status != LIBUSB_TRANSFER_COMPLETED) {
                        // Isochronous packets get lost (missed microframe, CRC). Keep the sample clock
                        // continuous with silence; only a second of nothing but losses is fatal.
                        if (++strmh->lost_in_row > strmh->packets_per_second) strmh->usbTransferError = UAC_ERROR_UNKNOWN;
                        uint8_t *pktbuf = libusb_get_iso_packet_buffer(transfer, packet_id);
                        memset(pktbuf, 0, strmh->silence_bytes);
                        strmh->cb_func(pktbuf, strmh->silence_bytes);
                        continue;
                    }
                    strmh->lost_in_row = 0;
    ...
    std::unique_ptr<const uac_audio_config_uncompressed> uac_stream_if_impl::query_config_uncompressed(
        ...
        std::unique_ptr<const uac_audio_config_uncompressed> best;
        for (auto&& setting : altsettings) {
            ...
            if ((audioDataFormatType == UAC_FORMAT_DATA_ANY || setting.general.wFormatTag == audioDataFormatType)
                ...
                && (best == nullptr || format1->bBitResolution > best->bBitResolution)) {
                best = std::make_unique<uac_audio_config_uncompressed>(
    ...
    void uac_stream_handle_impl::start(stream_cb_func stream_cb_func, int burst) {
        ...
        // High speed services endpoints per 125 us microframe; keep full speed's time per transfer.
        if (libusb_get_device_speed(libusb_get_device(dev_handle->usb_handle)) >= LIBUSB_SPEED_HIGH)
            packets_per_second = 8000u >> (std::clamp<uint8_t>(altsetting.endpoint.bInterval, 1, 4) - 1);
        const int iso_packets = burst * static_cast<int>(packets_per_second / 1000);
        silence_bytes = std::min<unsigned>(wMaxPacketSize, stride * (target_sampling_rate / packets_per_second));
        ...
        mActiveTransfers = 0;
        /* A transfer can complete immediately on a fast host. Mark the stream
         * active before submission so its callback resubmits that transfer. */
        active = true;
        for (int i = 0; i < NUM_ISO_TRANSFERS; ++i) {
            ...
            std::unique_lock submitLock(mMutex);
