...
namespace uac {
        ...
        for (size_t i = 0; i < configDesc->bNumInterfaces; ++i) {
            ...
            if (intf_desc->bInterfaceClass == LIBUSB_CLASS_AUDIO) {
                ...
                case uac_subclass_code::UAC_SUBCLASS_AUDIOCONTROL:
                    // A MIDI function brings its own AC interface without terminals; keep the audio one.
                    if (!audiocontrol || audiocontrol->outputTerminals.empty())
    ...
    std::unique_ptr<uac_audiocontrol> parse_audiocontrol(const libusb_interface_descriptor *ifdesc) {
        ...
        while (remaining >= 3) {
            ...
            case UAC_AC_OUTPUT_TERMINAL:
                audiocontrol->outputTerminals.push_back(parse_output_terminal(data, descSize, audiocontrol->bcdADC >= 0x0200));
                break;
            case UAC2_CLOCK_SOURCE:
            case UAC2_CLOCK_SELECTOR: {
                uac2_clock_entity clock{static_cast<uint8_t>(subtype), data[3], {}};
                for (int i = 0; subtype == UAC2_CLOCK_SELECTOR && descSize > 4 && i < data[4] && 5 + i < descSize; ++i)
                    clock.sources.push_back(data[5 + i]);
                audiocontrol->clocks.push_back(clock);
                break;
            }
    ...
    void scan_audiostreaming(uac_audiocontrol& ac, const libusb_interface *usbintf) {
        ...
        // UAC2 groups streaming interfaces by interface association instead of listing them in the header.
        if (uac2) ac.streams.emplace_back(ifdesc->bInterfaceNumber);
        for (auto &&stream : ac.streams) {
            if (stream.bInterfaceNr == ifdesc->bInterfaceNumber) {
                ...
                parse_audiostreaming_intf(stream, usbintf->altsetting, usbintf->num_altsetting, uac2);
                return;
            }
        }
    ...
    void parse_ac_header(uac_audiocontrol& ac, const uint8_t *data, int size) {
        ...
        if (ac.bcdADC >= 0x0200) { // UAC2: bCategory, wTotalLength, bmControls and no stream list
            ac.wTotalLength = TO_WORD(data+6);
            return;
        }
    ...
    std::shared_ptr<uac_output_terminal> parse_output_terminal(const uint8_t *data, int size, bool uac2) {
        ...
        terminal->bCSourceID = uac2 ? data[8] : 0;
        terminal->iTerminal = data[uac2 ? 11 : 8];
    ...
    void parse_as_general(uac_as_general &generalDesc, const uint8_t *data, int size, bool uac2) {
        ...
        if (uac2) { // bmControls, bFormatType, bmFormats bitmap, bNrChannels
            const uint8_t formats = data[5] == UAC_FORMAT_TYPE_I ? data[6] : 0;
            generalDesc.wFormatTag = formats & 0x01 ? UAC_FORMAT_DATA_PCM :
                    formats & 0x04 ? UAC_FORMAT_DATA_IEEE_FLOAT : UAC_FORMAT_DATA_TYPE_I_UNDEFINED;
            generalDesc.bNrChannels = data[10];
            return;
        }
    ...
    std::unique_ptr<uac_format_type_desc> parse_as_format_type(const uint8_t *data, int size, const uac_as_general &general, bool uac2) {
        if (uac2) {
            // UAC2 Type I has only bSubslotSize and bBitResolution. Rates belong to the clock entity,
            // so accept any here; start_streaming() verifies the rate on the clock.
            auto desc = (uac_format_type_1*) calloc(1, sizeof(uac_format_type_1) + sizeof(uint32_t));
            desc->bFormatType = data[3] == UAC_FORMAT_TYPE_I ? UAC_FORMAT_TYPE_I : UAC_FORMAT_TYPE_UNDEFINED;
            desc->bNrChannels = general.bNrChannels;
            desc->bSubframeSize = data[4];
            desc->bBitResolution = data[5];
            desc->tUpperSamFreq = UINT32_MAX;
            return std::unique_ptr<uac_format_type_desc>(desc);
        }
        ...
        for (size_t i = 1; i < num_altsetting; ++i) {
            ...
            while (remaining >= 3) {
                ...
                case UAC_AS_FORMAT_TYPE:
                    ...
                    altsetting.formatTypeDesc = parse_as_format_type(data, descSize, altsetting.general, uac2);
            ...
            // The data endpoint comes first; a second one is an explicit feedback endpoint.
            ...
            // High-bandwidth endpoints encode extra transactions per microframe in bits 11-12.
            const uint16_t maxPacket = ifdesc->endpoint->wMaxPacketSize;
            epDesc.wMaxPacketSize = (maxPacket & 0x7ff) * (1 + ((maxPacket >> 11) & 3));
