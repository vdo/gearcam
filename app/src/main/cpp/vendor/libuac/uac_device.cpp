...
namespace uac {
    ...
    std::vector<ref_uac_audio_route> uac_device_impl::query_audio_routes(uac_terminal_type termIn, uac_terminal_type termOut) const {
        ...
        for (auto&& aft : routes) {
            // The topology walk stops at units it does not parse (selector, mixer, UAC2 units).
            // ANY needs only the output terminal, so do not require the walk to reach an input.
            if (aft.contains_terminal_out(termOut) && (termIn == UAC_TERMINAL_ANY || aft.contains_terminal_in(termIn))) {
    ...
    /** UAC2 sets the rate on the clock source feeding the streaming terminal, not on the endpoint. */
    static void set_uac2_clock(libusb_device_handle *handle, const uac_audiocontrol &ac, uint8_t terminalId, uint32_t rate) {
        uint8_t id = 0;
        for (auto &&terminal : ac.outputTerminals)
            if (terminal->bTerminalID == terminalId) id = terminal->bCSourceID;
        for (int hop = 0; hop < 8; ++hop) { // follow clock selectors to the selected source
            auto clock = std::find_if(ac.clocks.begin(), ac.clocks.end(), [id](const uac2_clock_entity &c) { return c.bClockID == id; });
            if (clock == ac.clocks.end() || clock->subtype != UAC2_CLOCK_SELECTOR || clock->sources.empty()) break;
            uint8_t pin = 1; // CUR request (0x01) on CX_CLOCK_SELECTOR_CONTROL
            libusb_control_transfer(handle, REQ_TYPE_IF_GET, 0x01, 0x0100, id << 8 | ac.bInterfaceNumber, &pin, 1, 1000);
            id = clock->sources[pin >= 1 && pin <= clock->sources.size() ? pin - 1 : 0];
        }
        const uint16_t index = id << 8 | ac.bInterfaceNumber;
        uint8_t value[4] = {uint8_t(rate), uint8_t(rate >> 8), uint8_t(rate >> 16), uint8_t(rate >> 24)};
        // CUR on CS_SAM_FREQ_CONTROL. Fixed clocks refuse the write, so only the read-back decides.
        libusb_control_transfer(handle, REQ_TYPE_IF_SET, 0x01, 0x0100, index, value, sizeof(value), 1000);
        uint32_t actual = 0;
        for (int attempt = 0; attempt < 10; ++attempt) { // some clocks report the new rate only once locked
            if (libusb_control_transfer(handle, REQ_TYPE_IF_GET, 0x01, 0x0100, index, value, sizeof(value), 1000) == sizeof(value)) {
                actual = TO_DWORD(value);
                if (actual == rate) return;
            }
            std::this_thread::sleep_for(std::chrono::milliseconds(20));
        }
        throw uac_exception("USB clock runs at %u Hz, not %u Hz. Set the device to %u kHz.", actual, rate, rate / 1000);
    }
    ...
    std::shared_ptr<uac_stream_handle> uac_device_handle_impl::start_streaming(const uac_stream_if& streamIf, const uac_audio_config_uncompressed& config, stream_cb_func cb_func, int burst) {
        ...
        if (device->audiocontrol->bcdADC >= 0x0200)
            set_uac2_clock(usb_handle, *device->audiocontrol, altsetting.general.bTerminalLink, config.tSampleRate);
