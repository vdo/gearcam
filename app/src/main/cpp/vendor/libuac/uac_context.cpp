...
namespace uac {
    ...
    uac_context_impl::uac_context_impl(libusb_context *libusb_ctx) : libusb_ctx(libusb_ctx), owns_context(libusb_ctx == nullptr), alive(true) {
        LOG_DEBUG("create context with usb context: %p", libusb_ctx);
        if (owns_context) {
            /* Android grants access through UsbManager and an already-open fd. Do
             * not scan /dev/bus/usb, which is unavailable to ordinary apps. */
            libusb_set_option(nullptr, LIBUSB_OPTION_NO_DEVICE_DISCOVERY, nullptr);
            int errval = libusb_init(&this->libusb_ctx);
            if (errval != LIBUSB_SUCCESS) {
                throw usb_exception_impl("libusb_init()", static_cast<libusb_error>(errval));
            }
